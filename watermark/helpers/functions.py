...
def setup_argparser():
	...
	ap.add_argument("-k",	"--keep",				action="store_true",												help=f"keep original file format (if omitted, will default to { default_format.upper() } format)")
	ap.add_argument("-np",	"--no-prefix",			action="store_true",												help=f"do not add a '{ wm_prefix }' prefix to outputs")
	...
	ap.add_argument("-i",	"--input",				action="store", type=str,	default=default_input,					help=f"set a custom input directory path (default is '{ default_input }')")
	ap.add_argument("-o",	"--output",				action="store", type=str,	default=default_output,					help=f"set a custom output directory path (default is '{ default_output }')")
	ap.add_argument("-wms",	"--watermark-size",		action="store", type=float,	default=default_wm_size,				help=f"set the size of the watermark compared to the image's size (default is { default_wm_size })")
	ap.add_argument("-wmr",	"--watermark-ratio",	action="store", type=float,	default=default_wm_ratio,				help=f"set the size ratio between the logo's width and the circle's diameter, (default is { default_wm_ratio })")
	ap.add_argument("-wmp",	"--watermark-padding",	action="store", type=float,	default=default_wm_padding,				help=f"set the padding between the logo and the edge of the picture, as a ratio of the logo's height (default is { default_wm_padding })")
	ap.add_argument("-ss",  "--supersampling",		action="store", type=int,	default=default_ss, metavar="FACTOR",	help=f"set the supersampling factor for smoothing the circle (default is { default_ss }, smaller means faster execution but less smoothing)")
