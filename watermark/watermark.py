...
def export_image(img, big_img, factor, wm, pos, color, dist, file_out, circle, radius, suff=""):
	...
	if circle:
		...
		ImageDraw.Draw(big_img).ellipse(tuple(factor * x for x in (sW - r, sH - r, sW + r, sH + r)), fill=color)
	
	# Downsample
	img = big_img.resize(img.size)
...
for fn in fns:
	...
	big_img = img.resize([ss * x for x in img.size])

	vs, v_total = 0, len(this_pos) * len(this_color)
	...
	for p in this_pos:
		for suff, c in enumerate(this_color):
			...
			# Export the image
			print(proc_str.format(count, total, vs, v_total, invalid), end="\r")
			export_image(img, big_img.copy(), ss, this_wm, p, c, dist, file_out, circle=circle, radius=radius, suff=suff)
